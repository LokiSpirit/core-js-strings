...
function sumOfCodes(str) {
  if (!str) return 0;
  return Array.from(str).reduce((sum, item) => sum + item.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  return Array.from(str.toLowerCase()).reduce(
    (sum, val) => sum + (vowels.includes(val) ? 1 : 0),
    0
  );
}
...
function isPalindrome(str) {
  ...
  const toDelet = '!? ,.-:"\'`';
  const resStr = Array.from(newStr).reduce(
    (result, item) => result + (toDelet.includes(item) ? '' : item),
    ''
  );
  return resStr === Array.from(resStr).reverse().join('');
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce((res, item) => (res.length >= item.length ? res : item), '');
}
...
function encodeToRot13(str) {
  ...
  for (let s = 0; s < str.length; s += 1) {
    if (direct.includes(str[s])) {
      res.push(shifted.charAt(direct.indexOf(str[s])));
    } else {
      res.push(str[s]);
    }
  }
